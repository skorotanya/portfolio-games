...
// check duplicate number in row, col and box
...
//find unassined cell
const findUnassinedPos = (grid, pos) => {
	for (let row = 0; row < CONSTANT.GRID_SIZE; row++) {
		for (let col = 0; col < CONSTANT.GRID_SIZE; col++) {
...
const shuffleArray = (arr) => {
	...
	while(curr_index !== -1){
		...
		arr[rand_index] = temp;
		//console.table(curr_index, rand_index, arr[curr_index], arr[rand_index])
		curr_index -= 1;
	}
	//console.log(arr);
	return arr;
...
const sudokuCreate = (grid) => {
	...
	if(!findUnassinedPos(grid, unassinedPos)) return true;

	let numberList = shuffleArray([...CONSTANT.NUMBERS]);

	let row = unassinedPos.row;
	let col = unassinedPos.col;
...
const removeCells = (grid, level) => {
	...
	let res = [];
	for(let i = 0; i<CONSTANT.GRID_SIZE; i++) res[i] = [...grid[i]];
...
// generate cudoku based on level
